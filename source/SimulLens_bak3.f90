...
#include 'par.fh'
  ...
  type(vec2D) shift
  integer i,j,fu,i1,j1,icount,kx,ky,ir,index_nbody_run
  ...
  do ir=1,nr
     ...
     do j=1,nslice
        ...
        if(z_write(j).gt.0.017)then
          ...
          sublist_run(1:num_nbody_runs-2)=list_run(1*num_nbody_runs+1:2*num_nbody_runs-2)
