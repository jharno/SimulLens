...
Subroutine GaussRandomField_2d_r2c(den, nc, kg, delta2, nk, x1,x2)
!Subroutine GaussRandomField_2d_r2c(den, box, nc, kg, delta2, nk, x1,x2)
  ...
  !! nk is the number of points at the input power spectrum table
  !! box is the physical size of the box
  !! kg is the wavenumber of the input power spectrum
  !! delta2 is the power spectrum (either C_ell or l2Cl)
...
!  real, dimension(0:nc) :: factor
 ...
 !! generate whitenoise source
  ...
  write(*,*)'Got white noise', minval(x1),maxval(x2),sum(x1)
  write(*,*)'Got white noise', minval(x2),maxval(x1),sum(x2)
