...
112 close(12)
  ...
  subroutine writeXi
    ...
    fn1=PS_dir//'z_'//Z//'/Xi_'//RedShift//'_Run'//Run//'-'//Version//'.dat'

    !! Output powerer spectrum
...
#ifdef HALO
    fn1=Test_dir//'halopow_'//RedShift//'_Run'//Run//'-'//Version//'-shotnoise.dat'
#else    
    fn1=PS_dir//'z_'//Z//'/cicpow_'//RedShift//'_Run'//Run//'-'//Version//'-shotnoise.dat'
#endif
...
!!--------------------------------------------------------------!!
  ...
  subroutine GetXi
    ...
    !square the Fourier density
    d_COMPLEX = abs(d_COMPLEX*d_COMPLEX)    

    !Go back in real space to obtain Mass Auto-Correlation function
    call sfft3_r2c(d,nc,-1) 

    call Get_Average(d,Xi,nc)
...
!======================================================
 Subroutine Get_Average(d,Ave,nc)
!======================================================
! Calculates the Power Spectrum and Dumps it into a file


   Integer                            :: nc
   Real, Dimension(1:nc+2,1:nc,1:nc)  :: d
   Character(Len=120)              :: file

   !Real*4, Dimension(1:n+2,1:n,1:n):: fft
   Real, Dimension(3,1:nc)        :: pst
   Real, Dimension(nc)        :: Ave

   Real  :: w1,w2,kz,kx,ky,kr,pow
   Integer :: i,j,k,hn,k1,k2

   If (mod(nc,2) .NE. 0) Then
      Write(*,*) ' nc must be even'
      Stop
   End If
